...
video.addEventListener("ended", () => {
    gradiente.style.backgroundImage = "url('javascript9/bg-logo.svg')";
    gradiente.style.backgroundSize = "cover";
    gradiente.style.backgroundPosition = "center rigth";
    gradiente.style.backgroundRepeat = "no-repeat";
    gradiente.style.width = "100%"
...
let normas = JSON.parse(localStorage.getItem("normasPintura")) || [];

function salvarNorma() {
    let cliente = document.getElementById("clientePintura").value;
    let fundo = document.getElementById("fundoPintura").value;
    let acabamento = document.getElementById("acabamentoPintura").value;

    if (!cliente || !fundo || !acabamento) {
        alert("Preencha todos os campos!");
        return;
    }

    normas.push({cliente, fundo, acabamento});
    localStorage.setItem("normasPintura", JSON.stringify(normas));

    atualizarLista();

    document.getElementById("clientePintura").value = "";
    document.getElementById("fundoPintura").value = "";
    document.getElementById("acabamentoPintura").value = "";
}

function atualizarLista() {
    let select = document.getElementById("listaNormas");
    select.innerHTML = "<option value=''>Selecione uma norma</option>";

    normas.forEach((n, i) => {
        let opt = document.createElement("option");
        opt.value = i;
        opt.textContent = n.cliente;
        select.appendChild(opt);
    });
}

function mostrarNorma() {
    let i = document.getElementById("listaNormas").value;
    if (i === "") return;

    let n = normas[i];

    document.getElementById("normaSelecionada").value =
`Norma: ${n.cliente}
Fundo: ${n.fundo}
Acabamento: ${n.acabamento}`;
}

function copiarFundo() {
    let i = document.getElementById("listaNormas").value;
    if (i === "") return alert("Selecione uma norma");

    navigator.clipboard.writeText(normas[i].fundo);
    alert("Fundo copiado!");
}

function copiarAcabamento() {
    let i = document.getElementById("listaNormas").value;
    if (i === "") return alert("Selecione uma norma");

    navigator.clipboard.writeText(normas[i].acabamento);
    alert("Acabamento copiado!");
}
...
function removerNorma() {
    ...
    if (!confirm("Deseja remover essa norma?")) return;
